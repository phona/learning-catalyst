  ...
  stream: vi.fn().mockImplementation(async function* (messages: any[]) {
    ...
    for (const chunk of chunks) {
      yield {
        ...
        response_metadata: { model: this.model, usage: { total_tokens: 50 } }
      };
  ...
  stream: vi.fn().mockImplementation(async function* (messages: any[]) {
    ...
    for (const chunk of chunks) {
      yield {
        ...
        response_metadata: { model: this.model, usage: { total_tokens: 60 } }
      };
      await new Promise(resolve => setTimeout(resolve, 15)); // Simulate Anthropic streaming
    }
  }),
...
export const mockChatGLM = vi.fn().mockImplementation((config: any) => ({
  ...
  // Mock thinking process for ChatGLM
  stream: vi.fn().mockImplementation(async function* (messages: any[]) {
    ...
    yield {
      ...
      response_metadata: { model: this.model, stage: 'thinking' }
    };
    ...
    for (const chunk of chunks) {
      yield {
        ...
        response_metadata: { model: this.model, stage: 'response' }
      };
  ...
  invoke: vi.fn().mockImplementation(async (messages: any[]) => {
    ...
    return {
      ...
      response_metadata: { model: this.model, usage: { total_tokens: 80 } }
    };
  })
}));
...
export const mockChatPromptTemplate = vi.fn().mockImplementation((config: any) => ({
  template: config?.template || 'Template: {input}',
  ...
  formatMessages: vi.fn().mockImplementation((values: any) => {
    const formatted = this.format(values);
    return [
      { type: 'system', content: formatted }
    ];
  }),
...
export const mockLLMChain = vi.fn().mockImplementation((config: any) => ({
  llm: config?.llm || mockChatOpenAI({}),
  prompt: config?.prompt || mockChatPromptTemplate({ template: 'Default: {input}' }),

  invoke: vi.fn().mockImplementation(async (input: any) => {
    const formattedPrompt = await this.prompt.format(input);
    const response = await this.llm.invoke([
      { type: 'system', content: formattedPrompt }
    ]);
    return response;
  }),
  ...
  memory: config?.memory || mockBufferMemory(),
  ...
  splitDocuments: vi.fn().mockImplementation(async (documents: any[]) => {
    ...
    documents.forEach(doc => {
      ...
      for (let i = 0; i < content.length; i += size - overlap) {
        chunks.push(mockDocument(
          content.slice(i, i + size),
          { ...doc.metadata, chunkIndex: Math.floor(i / (size - overlap)) }
        ));
      }
    });
...
export const mockCallbackHandler = vi.fn().mockImplementation(() => ({
  ...
  onChainStart: vi.fn().mockImplementation(async (serialized, inputs) => {
    this.chains.push({ type: 'start', serialized, inputs, timestamp: Date.now() });
  }),

  onChainEnd: vi.fn().mockImplementation(async (outputs) => {
    this.chains.push({ type: 'end', outputs, timestamp: Date.now() });
  }),

  onChainError: vi.fn().mockImplementation(async (error) => {
    this.errors.push({ type: 'chain', error, timestamp: Date.now() });
  }),

  onLLMStart: vi.fn().mockImplementation(async (serialized, prompts) => {
    this.llms.push({ type: 'start', serialized, prompts, timestamp: Date.now() });
  }),

  onLLMEnd: vi.fn().mockImplementation(async (outputs) => {
    this.llms.push({ type: 'end', outputs, timestamp: Date.now() });
  }),

  onLLMError: vi.fn().mockImplementation(async (error) => {
    this.errors.push({ type: 'llm', error, timestamp: Date.now() });
  }),

  onToolStart: vi.fn().mockImplementation(async (serialized, input) => {
    this.tools.push({ type: 'start', serialized, input, timestamp: Date.now() });
  }),

  onToolEnd: vi.fn().mockImplementation(async (output) => {
    this.tools.push({ type: 'end', output, timestamp: Date.now() });
  }),

  onToolError: vi.fn().mockImplementation(async (error) => {
    this.errors.push({ type: 'tool', error, timestamp: Date.now() });
  }),

  onAgentAction: vi.fn().mockImplementation(async (action) => {
    this.agents.push({ type: 'action', action, timestamp: Date.now() });
  }),

  onAgentFinish: vi.fn().mockImplementation(async (finish) => {
    this.agents.push({ type: 'finish', finish, timestamp: Date.now() });
  }),
