# Component Architecture - Specification Delta

## Purpose
This specification enforces the split of the monolithic `LocalProjectExplorer` component (1045 lines) into focused, maintainable components with clear service boundaries and single responsibility.
...
### Modified Requirement: Component Responsibility (from renderer-structure spec)

**From**: Components can have multiple responsibilities
**To**: Components MUST have single responsibility
...
### Modified Requirement: Import Direction Rules (from renderer-structure spec)

**From**: Generic import rules
**To**: Strict separation of feature dependencies

**Rationale**: Prevent coupling between features

**Impact**:
- Discovery feature cannot import from Chat feature
- All features use shared ConfigurationService
- Clear service boundaries

## REMOVED Requirements

### Removed Requirement: Monolithic Components Allowed

**Rationale**: Replaced with single responsibility principle

**Replacement**: Components must be focused and <300 lines

### Removed Requirement: Cross-Feature Dependencies

**Rationale**: Creates tight coupling and maintenance issues

**Replacement**: Use shared ConfigurationService for cross-cutting concerns
...
```
src/renderer/features/discovery/ui/
├── index.ts                          # Barrel exports
├── LocalProjectExplorer.tsx          # NEW: Container (150 lines)
├── FileTree.tsx                      # NEW: Directory browser (200 lines)
├── FileSelector.tsx                  # NEW: Selection UI (150 lines)
├── ConceptParser.tsx                 # NEW: Parsing logic (300 lines)
├── ProviderStatus.tsx                # NEW: Provider validation (100 lines)
├── ParsingResultsModal.tsx           # NEW: Results modal (200 lines)
└── __tests__/                        # Tests follow components
    ├── LocalProjectExplorer.test.tsx
    ├── FileTree.test.tsx
    ├── FileSelector.test.tsx
    ├── ConceptParser.test.tsx
    ├── ProviderStatus.test.tsx
    └── ParsingResultsModal.test.tsx
```
...
ProviderStatus (100 lines)
  └─ useConfigurationService() ✓ (NOT ChatService!)
...
1. **Create FileTree** - Extract directory browsing (4 hours)
2. **Create FileSelector** - Extract selection logic (3 hours)
3. **Create ConceptParser** - Extract parsing (6 hours)
4. **Create ProviderStatus** - Extract provider validation (3 hours)
5. **Create ParsingResultsModal** - Extract modal (4 hours)
6. **Wire LocalProjectExplorer** - Create container (2 hours)
7. **Delete old implementation** - Remove 1045-line file (0.5 hours)
8. **Update tests** - Test all components (8 hours)
...
wc -l src/renderer/features/discovery/ui/*.tsx

# Verify no ChatService in Discovery
grep -r "useChatService" src/renderer/features/discovery/ --include="*.tsx"

# Verify ConfigurationService usage
grep -r "useConfigurationService" src/renderer/features/discovery/ --include="*.tsx"

# Run tests
npm run test:renderer

# Expected: All tests pass
# Expected: All components under size limits
# Expected: No cross-feature dependencies
```
...
1. **Keep old file**: Don't delete LocalProjectExplorer.tsx until new one passes all tests
2. **Incremental migration**: Create one component at a time, test, then move to next
3. **Version control**: Commit after each component extraction
4. **Test safety**: Run tests after each change

```bash
# If issues arise:
git checkout HEAD -- src/renderer/features/discovery/ui/LocalProjectExplorer.tsx
...
- [ConfigurationService](../../src/renderer/services/configuration/configuration-service.ts)
- [Current LocalProjectExplorer](../../src/renderer/features/discovery/ui/LocalProjectExplorer.tsx)
