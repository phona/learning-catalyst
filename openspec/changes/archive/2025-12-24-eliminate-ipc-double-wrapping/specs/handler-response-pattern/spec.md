# Spec: Handler Response Pattern

**Capability**: IPC Handler Response Pattern
**Change ID**: `eliminate-ipc-double-wrapping`

## Purpose

Establish a consistent pattern for IPC handler responses where handlers return raw data and the `ipc-main-proxy` handles all response wrapping. This eliminates double-wrapping bugs and simplifies handler code.
...
1. ✅ All 3 affected handler files (`sessions-handlers.ts`, `chat-handlers.ts`, `concept-parsing-handlers.ts`) return raw data
2. ✅ No `createSuccessResponse` or `createErrorResponse` functions exist in handler files
3. ✅ No `APIResponse` imports in handler files
4. ✅ No inline `return { success: true/false, ... }` patterns in handlers
5. ✅ History sessions display correctly in sidebar
6. ✅ All handler tests pass with updated expectations
7. ✅ Integration tests pass
8. ✅ Error handling still works correctly (errors thrown, not wrapped)
9. ✅ No regressions in existing features
10. ✅ Documentation updated with correct pattern

## Migration Notes

### Files Modified

1. `src/main/handlers/sessions-handlers.ts` - Remove wrappers, update ~9 handlers
2. `src/main/handlers/chat-handlers.ts` - Remove wrappers, update ~1 handler
3. `src/main/handlers/concept-parsing-handlers.ts` - Remove inline error returns, update ~2 handlers

### Tests Updated

1. `src/main/handlers/__tests__/sessions-handlers.test.ts` - Expect raw data
2. `src/main/handlers/__tests__/chat-handlers.test.ts` - Expect raw data
3. `src/main/handlers/__tests__/concept-parsing-handlers.test.ts` - Expect raw data

### Breaking Changes

None for the renderer process. The `unwrapAPI` helper remains unchanged - it still expects and unwraps `APIResponse<T>`. The change is isolated to handler return values.
