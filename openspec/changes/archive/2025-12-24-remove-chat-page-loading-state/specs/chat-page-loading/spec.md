...
Defines requirements for the ChatPage component rendering behavior. The ChatPage component must render the assistant-ui `<Thread>` component immediately without blocking, delegating loading and empty state handling to the assistant-ui library's built-in capabilities.
...
- **chat-navigation-from-sidebar**: Defines sidebar click navigation behavior, which works in conjunction with ChatPage URL synchronization
- **component-architecture**: Defines renderer component structure and patterns
