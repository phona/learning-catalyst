...
class SiliconFlowProvider:
    ...
    async def validate_credentials(self, credentials: Credentials) -> bool:
        """Validate SiliconFlow API credentials"""
        try:
            headers = {
    ...
    async def list_available_models(self) -> List[str]:
        """Get list of available SiliconFlow models"""
        headers = {
            "Authorization": f"Bearer {self.api_key}",
            "Content-Type": "application/json",
        }
    ...
    async def get_provider_capabilities(self) -> ProviderCapabilities:
        """Get capabilities information for SiliconFlow provider"""
        return ProviderCapabilities(
            supports_streaming=True,
            max_tokens=32768,
            supported_models=await self.list_available_models(),
            input_cost_per_token=0.0000005,  # Example cost
            output_cost_per_token=0.000001,  # Example cost
