...
export const setupSettingsHandlers = (workspacePath?: string): void => {
  ...
  ipcMain.handle('settings:update-preferences', async (_event, preferences: Partial<UserPreferences>) => {
  ...
  ipcMain.handle('settings:configureProvider', async (_event, params: { provider: string; config: ProviderConfig }) => {
  ...
  ipcMain.handle('settings:update-learning-settings', async (_event, settings: Partial<LearningSettings>) => {
