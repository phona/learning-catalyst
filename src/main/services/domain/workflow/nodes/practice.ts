 ...
 * - Triggers: PresentProblem (Practice Agent: Present Problem)
 *
 * Purpose:
 ...
 * @returns Updated state with practice prompt and assistant message
 */
export const practiceNode = (deps: WorkflowDeps) => async (state: typeof WorkflowStateAnnotation.State) => {
  ...
  // ============================================================================
  // Return the formatted practice prompt and add assistant message to state
  // This allows the workflow to continue and the UI to display the practice
  return {
    practicePrompt,
    messages: [{ role: 'assistant', content: practicePrompt }],
  };
};
