...
import { Database, CheckpointRow, CheckpointWriteRow, CheckpointBlobRow, InsertableCheckpoint, InsertableCheckpointWrite, InsertableCheckpointBlob } from '@/main/services/core/database'
...
export class SQLiteCheckpointSaver extends BaseCheckpointSaver<number> {
  ...
  async put(
    ...
    try {
      ...
      await this.db.transaction().execute(async (trx) => {
        ...
        await trx.insertInto('checkpoints').values({
          id: uuidv4(),
          thread_id: threadId,
        ...
        if (newVersions) {
          for (const [channel, version] of Object.entries(newVersions)) {
            ...
            } as InsertableCheckpointWrite).execute()
          }
        }
      })
  ...
  async get(config: RunnableConfig): Promise<Checkpoint | undefined> {
    ...
    if (!checkpointId) {
      ...
      const latestTuple = await this.getLatestCheckpointTuple(threadId, config.configurable?.checkpoint_ns)
  ...
  async putWrites(
    ...
    try {
      for (const write of writes) {
        ...
        await this.db.insertInto('checkpoint_writes').values({
          id: uuidv4(),
          checkpoint_id: checkpointId,
          task_id: taskId,
          channel,
   ...
   */
  private matchesFilter(checkpoint: CheckpointTuple, filter: Record<string, any>): boolean {
  ...
  async putBlobs(
    ...
    try {
      for (const blob of blobs) {
        await this.db.insertInto('checkpoint_blobs').values({
          id: uuidv4(),
          checkpoint_id: checkpointId,
          task_id: taskId,
          channel: channel,
          ...
          created_at: new Date().toISOString()
        } as InsertableCheckpointBlob).execute()
      }
    } catch (error) {
