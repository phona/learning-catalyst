...
import { ServiceDependencies } from '@/main/services/types';
import {
  ConversationContext,
  PracticeOpportunity,
  UserContext,
...
export class ConversationAnalyzer {
  ...
  private readonly analysisCache = new Map<
    string,
    { result: ConversationAnalysisResult; timestamp: number }
  >();
  ...
  async analyzeConversation(
    ...
    try {
      ...
      const { maxMessages = 50, includeSystemMessages = false, analyzeSentiment = true } = options;
  ...
  private calculateMessageConfidence(content: string, isUserMessage: boolean): number {
    ...
    const confidenceIndicators = [
      {
        pattern: /\b(I understand|got it|makes sense|clear now|I get|that's clear)\b/gi,
        weight: 0.3,
      },
  ...
  private identifyLearningIndicators(content: string, isUserMessage: boolean): string[] {
    ...
    const learningPatterns = [
      {
        pattern: /\b(I understand|I get it|makes sense|clear now|got it)\b/gi,
        indicator: 'understanding',
      },
      {
        pattern: /\b(I don't understand|not clear|confused|uncertain|not sure)\b/gi,
        indicator: 'confusion',
      },
      {
        pattern: /\b(aha!|eureka!|breakthrough|suddenly|now I see)\b/gi,
        indicator: 'breakthrough',
      },
  ...
  ): ConversationContext['keyConcepts'] {
  ...
  private generatePracticeOpportunities(
    ...
    highReadinessConcepts.forEach((conceptData, index) => {
      const vibe = this.detectVibeFromAnalysis(messageAnalyses.slice(-5), conceptData.concept);
  ...
  private detectVibeFromAnalysis(analyses: MessageAnalysis[], concept: string): VibeType {
    ...
    const avgPracticeReadiness =
      recentAnalyses.reduce((sum, a) => sum + a.practiceReadiness, 0) / recentAnalyses.length;
    const avgConfidence =
  ...
  private shouldCreatePracticeOpportunity(vibe: VibeType, userContext: UserContext): boolean {
    ...
    if (userContext.lastPracticeTime) {
      ...
      if (timeSincePractice < 30 * 60 * 1000) {
        // 30 minutes
        return false;
      }
    }
  ...
  private calculateEngagementLevel(messageAnalyses: MessageAnalysis[]): number {
    ...
    const avgEngagement =
      messageAnalyses.reduce((sum, a) => sum + a.engagement, 0) / messageAnalyses.length;

    // Recent messages weigh more heavily
    const recentMessages = messageAnalyses.slice(-5);
    const recentEngagement =
      recentMessages.reduce((sum, a) => sum + a.engagement, 0) / recentMessages.length;
  ...
  private calculateLearningVelocity(
    ...
    const conceptsIntroduced = messageAnalyses.reduce((set, analysis) => {
