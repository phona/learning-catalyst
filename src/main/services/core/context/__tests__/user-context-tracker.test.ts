...
import { createUserContextTracker } from '@/main/services/core/context/user-context-tracker';
import { ContextUpdateRequest, UserContext } from '@/shared/types/practice';

describe('UserContextTracker', () => {
  let userContextTracker: ReturnType<typeof createUserContextTracker>;
  ...
  describe('Practice History', () => {
    it('should maintain complete practice history', async () => {
      ...
      expect(currentContext.practiceHistory[0].topic).toBe('concept1');
      expect(currentContext.practiceHistory[0].success).toBe(true);
      expect(currentContext.practiceHistory[1].topic).toBe('concept2');
  ...
  describe('Project Context', () => {
    it('should allow setting and retrieving project context', () => {
      ...
      const currentContext = userContextTracker.getCurrentContext();
      expect(currentContext.currentProject).toEqual({
        name: 'My React App',
        type: 'react',
        files: ['/src/App.tsx'],
