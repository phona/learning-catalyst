...
export const createUserContextTracker = (
  ...
  const updateCognitiveMetrics = (request: ContextUpdateRequest, success?: boolean): void => {
    const recentConceptsCount = recentActivity.filter(a => a.timestamp > Date.now() - 3600000).length;
    cognitiveMetrics.currentVelocity = Math.max(0.1, recentConceptsCount / 10);

    if (success !== undefined) {
      const retentionChange = success ? 0.02 : -0.01;
      cognitiveMetrics.retentionRate = Math.max(0.1, Math.min(1,
        cognitiveMetrics.retentionRate + retentionChange));
    }

    const timeSinceStart = (Date.now() - (recentActivity[0]?.timestamp ?? Date.now())) / 3600000;
    cognitiveMetrics.fatigueLevel = Math.min(1, timeSinceStart * 0.1 + currentContext.cognitiveLoad * 0.3);

    const motivationChange = (success ? 0.05 : -0.02) +
      (currentContext.engagementLevel - 0.5) * 0.1;
    cognitiveMetrics.motivationLevel = Math.max(0.1, Math.min(1,
      cognitiveMetrics.motivationLevel + motivationChange));
  ...
  const generateRecommendations = (_changes: ContextChanges): Recommendation[] => {
    ...
    const practiceReady = currentContext.confidenceLevel > 0.7 && currentContext.engagementLevel > 0.6;
    ...
    const highConfidenceConcepts = currentContext.recentConcepts.filter(concept => concept.confidence > 0.85);
  ...
  const handleLearningPatternDetected = (_request: ContextUpdateRequest, changes: ContextChanges): void => {
  ...
  const handleConceptIntroduction = (request: ContextUpdateRequest, changes: ContextChanges): void => {
    const concepts = request.concepts ?? [];
    const confidence = request.confidence ?? 0.5;

    if (request.sentiment !== undefined) {
      currentContext.engagementLevel = Math.max(currentContext.engagementLevel, (request.sentiment + 1) / 2);
    }
    ...
    for (const concept of concepts) {
      ...
      if (!existingConcept) {
        ...
        changes.learningPatterns.push(pattern);
      } else {
  ...
  const handlePracticeCompletion = (request: ContextUpdateRequest, changes: ContextChanges): void => {
    ...
    if (!conceptRecord) {
      const { record, pattern } = addConceptRecord(concept, request.confidence ?? 0.5, request.timestamp);
    ...
    currentContext.confidenceLevel = Math.max(currentContext.confidenceLevel, conceptRecord.confidence);
    ...
    if (activePattern) {
      ...
      const successRate = activePattern.practiceAttempts > 0
        ? (practiceHistory.filter(p => p.concept === concept && p.success).length / activePattern.practiceAttempts)
        : 0;
      activePattern.successRate = successRate;

      if (successRate >= 0.8 && activePattern.confidenceProgression[activePattern.confidenceProgression.length - 1] > 0.8) {
  ...
  const handleMessage = (request: ContextUpdateRequest, changes: ContextChanges): void => {
    ...
    currentContext.cognitiveLoad = Math.min(1, currentContext.cognitiveLoad + (messageComplexity * 0.1));
    ...
    try {
      ...
      try {
        ...
        handleLearningPatternDetected(request, {
          ...
          newStuckPoints: [],
          resolvedStuckPoints: [],
          learningPatterns: [...learningPatterns, ...patternResult.patterns]
        });
      } catch (error) {
  ...
  const getCurrentContext = (): UserContext => {
    ...
    const transformedProjectContext = projectContext ? {
      ...
      recentActivity: projectContext.recentFiles.map(file => `${file.path}: ${file.concepts.join(', ')}`)
    } : undefined;
    ...
    return {
      ...
      lastPracticeTime: practiceHistory.length > 0
      ...
      statistics: {
        ...
        averageSessionLength: practiceHistory.length > 0
  ...
  const setProjectContext = (projectContextUpdate: UserContext['currentProject']): void => {
    projectContext = projectContextUpdate ? {
      ...
      recentFiles: projectContextUpdate.files?.map(file => ({
        path: file,
  ...
  const getLearningAnalytics = () => {
    const confidenceTrend = recentActivity
      .slice(-20)
      .map(a => a.confidence);

    const successRateTrend = practiceHistory
      .slice(-10)
      .map(() => getRecentSuccessRate());

    const velocityTrend = recentActivity
      .slice(-20)
      .map((_, index) => {
