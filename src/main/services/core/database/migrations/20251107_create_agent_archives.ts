  ...
  async up(db: Kysely<unknown>): Promise<void> {
    ...
    await db.schema
      ...
      .addColumn('archive_reason', 'text', (col) =>
        col
          .defaultTo('deletion')
          .check(sql`archive_reason IN ('deletion', 'migration', 'backup', 'maintenance')`),
      )
      .addColumn('retained_history', 'boolean', (col) => col.defaultTo(false))
      .addColumn('archived_at', 'integer', (col) =>
        col.notNull().defaultTo(sql`strftime('%s', 'now')`),
      )
      .addColumn('expires_at', 'integer') // Optional expiration timestamp
      .addForeignKeyConstraint('fk_agent_archives_agent_id', ['agent_id'], 'agents', ['id'], (fk) =>
        fk.onDelete('cascade'),
      )
      .execute();
  },
