 ...
 */
export const createProviderService = (config: ProviderConfig) => ({
  loggerService
}: {
  ...
  return {
    chatCompletion: async (params: ChatCompletionParams): Promise<ChatCompletionResult> => {
      ...
      const rawResponse = {
        ...
        usage: {
          prompt_tokens: params.messages.reduce((sum: number, msg: ChatMessage) => sum + msg.content.length, 0),
          completion_tokens: config.defaultCompletionTokens,
          total_tokens: params.messages.reduce((sum: number, msg: ChatMessage) => sum + msg.content.length, 0) + config.defaultCompletionTokens
        }
      };
    ...
    validateConfig: (modelConfig: ModelConfig): boolean => {
      ...
      try {
        ...
        if (config.requiresApiKey && (!modelConfig.apiKey || typeof modelConfig.apiKey !== 'string')) {
