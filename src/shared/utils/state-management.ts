 ...
 */
export type TypedStoreCreator<TState extends BaseStoreState, TActions extends BaseStoreActions<TState>> = 
  StateCreator<TState & TActions, [["zustand/subscribeWithSelector", never]], [], TState & TActions>;
    ...
    (set: any, get: any, api: StoreApi<T>) => {
      ...
      return {
        ...
        set: (newState: any) => {
          ...
          if (duration > 16) { // Log slow operations (> 16ms)
 ...
 */
export function createBaseStore<TState extends BaseStoreState, TActions extends BaseStoreActions<TState>>(
  storeName: string,
  ...
  if (options.validation) {
    config = createValidationMiddleware(options.validation)(config) as TypedStoreCreator<TState, TActions>;
  }
  
  // Add debug middleware if enabled
  if (options.enableDebug) {
    config = createDebugMiddleware<TState & TActions>(storeName)(config) as TypedStoreCreator<TState, TActions>;
  }
  
  // Add performance monitoring if enabled
  if (options.enablePerformanceMonitoring) {
    config = createPerformanceMiddleware<TState & TActions>(storeName)(config) as TypedStoreCreator<TState, TActions>;
  }
  ...
  if (options.persistence) {
    config = persist(config, options.persistence);
  }
  
  return zustandCreate<TState & TActions>()(config);
}

/**
 * Standard action creators factory
 */
export function createActionCreators<TState extends BaseStoreState, TActions extends BaseStoreActions<TState>>(
  actions: TActions
): TActions {
  return actions;
}
 ...
 */
export function createSelectors<TStore extends { getState: () => any }>(store: TStore) {
  return {
    get: store.getState,
...
export class StoreLifecycleManager {
  ...
  async initializeAllStores(): Promise<void> {
    const initializationPromises = Array.from(this.stores.entries()).map(async ([name, { lifecycle }]) => {
      try {
        await lifecycle.initialize();
        console.debug(`Store '${name}' initialized successfully`);
      } catch (error) {
        console.error(`Failed to initialize store '${name}':`, error);
        lifecycle.setError(`Initialization failed: ${error instanceof Error ? error.message : String(error)}`);
      }
    });
  ...
  static createDerivedStore<TState>(
    ...
    return {
      subscribe: (callback: (state: any) => void) => {
        const unsubscribers = Object.values(dependencies).map(store => 
