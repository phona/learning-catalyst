...
const AchievementsComponent: React.FC<AchievementsProps> = ({ analytics, className = '' }) => {
  ...
  const getAchievementIcon = useCallback(
    (achievement: Achievement, size: 'small' | 'medium' | 'large' = 'medium') => {
      const sizeClasses = {
        small: 'w-4 h-4',
        medium: 'w-6 h-6',
        large: 'w-8 h-8',
      ...
      return (
        ...
        >
          {achievement.icon ? (
            <span
              className={`${size === 'small' ? 'text-lg' : size === 'medium' ? 'text-2xl' : 'text-3xl'}`}
            >
              {achievement.icon}
            </span>
          ) : (
  ...
  const getCategoryGradient = (category: Achievement['category']): string => {
    const gradients: Record<Achievement['category'], string> = {
      ...
      concepts:
        'from-purple-100 to-purple-200 dark:from-purple-900/30 dark:to-purple-800/30 text-purple-800 dark:text-purple-200',
      streaks:
        'from-orange-100 to-orange-200 dark:from-orange-900/30 dark:to-orange-800/30 text-orange-800 dark:text-orange-200',
      performance:
        'from-emerald-100 to-emerald-200 dark:from-emerald-900/30 dark:to-emerald-800/30 text-emerald-800 dark:text-emerald-200',
      engagement:
        'from-pink-100 to-pink-200 dark:from-pink-900/30 dark:to-pink-800/30 text-pink-800 dark:text-pink-200',
    };
  ...
  if (loading) {
    return (
      <div
        className={`bg-gradient-to-br from-white to-gray-50/80 dark:from-gray-800 dark:to-gray-900/80 rounded-2xl border border-gray-200/60 dark:border-gray-700/60 p-8 backdrop-blur-sm shadow-lg ${className}`}
      >
  ...
  if (error != null && error !== '') {
    return (
      <div
        className={`bg-gradient-to-br from-red-50/80 to-orange-50/80 dark:from-red-900/20 dark:to-orange-900/20 rounded-2xl border border-red-200/60 dark:border-red-800/60 p-8 backdrop-blur-sm shadow-lg ${className}`}
      >
        <div className="text-center">
  ...
  return (
    <div
      className={`bg-gradient-to-br from-white to-gray-50/80 dark:from-gray-800 dark:to-gray-900/80 rounded-2xl border border-gray-200/60 dark:border-gray-700/60 p-6 backdrop-blur-sm shadow-lg ${className}`}
    >
      ...
      <div className="mb-8 p-4 bg-gradient-to-br from-gray-50 to-white dark:from-gray-900 dark:to-gray-800 rounded-xl border border-gray-200/60 dark:border-gray-700/60">
        ...
        {overallProgress >= 25 && (
          <div className="mt-2 text-xs font-medium animate-fade-in">
            {overallProgress >= 100 && (
              <span className="text-emerald-600 dark:text-emerald-400">{`🎉 Achievement Master! You've unlocked everything!`}</span>
            )}
            {overallProgress >= 75 && overallProgress < 100 && (
              <span className="text-blue-600 dark:text-blue-400">{`🌟 So close! You're almost there!`}</span>
            )}
            {overallProgress >= 50 && overallProgress < 75 && (
              <span className="text-purple-600 dark:text-purple-400">{`🚀 Great progress! You're halfway there!`}</span>
            )}
            {overallProgress >= 25 && overallProgress < 50 && (
              <span className="text-amber-600 dark:text-amber-400">💪 Good start! Keep going!</span>
            )}
          </div>
        )}
      </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-3">
                      <span
                        className={`text-xs px-2 py-1 rounded-full bg-gradient-to-r ${getCategoryGradient(achievement.category)} font-medium`}
                      >
      ...
      {(achievements?.length ?? 0) === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">
            Start your learning journey to unlock amazing rewards!
          </p>
