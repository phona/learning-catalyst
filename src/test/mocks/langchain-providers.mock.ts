...
class MockResponseGenerator {
  ...
  static getRandomResponse(provider: string): string {
    const providerResponses =
      this.responses[provider as keyof typeof this.responses] || this.responses.openai;
...
export class LangChainProviderMockFactory {
  private static readonly mocks = new Map<string, any>();
  ...
  // Explicit helpers for common providers
  static createOpenAIMock(config: ProviderMockConfig = {}) {
  ...
  static createChatGLMMock(config: ProviderMockConfig = {}) {
   ...
   */
  static createIntermittentErrorProvider(
    provider: string,
