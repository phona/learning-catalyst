...
describe('Enhanced Provider Mocking', () => {
  ...
  it('should create ChatGLM provider mock', () => {
    ...
    it('should handle streaming calls', async () => {
      ...
      for await (const chunk of glmMock.stream(messages)) {
        chunks.push(chunk);
      }
  ...
  describe('Error Scenario Testing', () => {
    it('should simulate timeout errors', async () => {
      ...
      await expect(timeoutMock.invoke([{ role: 'user', content: 'test' }])).rejects.toThrow(
        'Request timeout',
      );
    });
    ...
    it('should simulate authentication errors', async () => {
      ...
      await expect(authErrorMock.invoke([{ role: 'user', content: 'test' }])).rejects.toThrow(
        'Invalid API key',
      );
    });
  ...
  describe('Token Usage Simulation', () => {
    it('should simulate high token usage', async () => {
      const highTokenMock = LangChainProviderMockFactory.createOpenAIMock(
        ProviderConfigs.HIGH_TOKEN_USAGE,
      );
    ...
    it('should simulate low token usage', async () => {
      const lowTokenMock = LangChainProviderMockFactory.createOpenAIMock(
        ProviderConfigs.LOW_TOKEN_USAGE,
      );
  ...
  describe('Mock Configuration and Customization', () => {
    ...
    it('should reset all mocks', async () => {
      ...
      const glmMock = LangChainProviderMockFactory.getProvider('chatglm');

      // Make calls
      await openaiMock.invoke([{ role: 'user', content: 'test' }]);
      await glmMock.invoke([{ role: 'user', content: 'test' }]);

      // Check stats before reset
      let openaiStats = LangChainProviderMockFactory.getCallStats('openai');
      let glmStats = LangChainProviderMockFactory.getCallStats('chatglm');
      expect(openaiStats?.invokeCount).toBe(1);
      expect(glmStats?.invokeCount).toBe(1);
      ...
      glmStats = LangChainProviderMockFactory.getCallStats('chatglm');
      expect(openaiStats).toBeNull();
      expect(glmStats).toBeNull();
    });
  });
