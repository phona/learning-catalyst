...
describe('Cross-Process Communication Integration Tests', () => {
  ...
  describe('MessageChannelMain Streaming', () => {
    it('should handle bidirectional streaming communication', async () => {
      ...
      ipcHandlers.set('catalyst:start-stream', async (event, request) => {
        ...
        // Send port2 back to renderer immediately
        setTimeout(() => {
        ...
        }, 25); // Reduced interval for faster test execution
    ...
    it('should handle stream interruption and recovery', async () => {
      ipcHandlers.set('catalyst:interruptible-stream', async (event, request) => {
        ...
        // Send port message with a small delay
        setTimeout(() => {
          event.sender.postMessage('catalyst:stream-port', { port: port2 }, [port2]);
        }, 5);

        let interrupted = false;
        const streamData = Array.from({ length: 10 }, (_, i) => `Chunk ${i}: Learning content part ${i}`); // Reduced size
      ...
      const streamInterrupted = new Promise<void>((resolve, reject) => {
        ...
        mockRendererProcess.once('catalyst:stream-port', (event, { port }) => {
          ...
          port.on('message', (message: IPCStreamMessage) => {
            if (message.type === 'chunk') {
              receivedChunks.push(message);
              // Interrupt after receiving 2 chunks (reduced for faster test)
      ...
      // Add resume-stream handler
      ipcHandlers.set('catalyst:resume-stream', async (event, request) => {
        return {
          success: true,
          streamId: request.streamId,
          resumedFrom: request.fromChunk,
    ...
    it('should handle concurrent streams efficiently', async () => {
      // Set up multiple concurrent streams (reduced count for test reliability)
      const streamCount = 3;
      ...
      ipcHandlers.set('catalyst:concurrent-stream', async (event, request) => {
        ...
        // Send port message immediately with a small delay
        setTimeout(() => {
          event.sender.postMessage(`catalyst:stream-port-${request.streamId}`, { port: port2 }, [port2]);
        }, 5);

        const streamData = Array.from({ length: 5 }, (_, i) => `Stream ${request.streamId} - Chunk ${i}`); // Reduced data size
        let index = 0;
      ...
      // Verify concurrent performance (relaxed threshold)
  ...
  describe('Error Handling and Recovery', () => {
    ...
    it('should handle timeout scenarios appropriately', async () => {
      ...
      // invokeWithTimeout should throw a timeout error
      await expect(
        mockRendererProcess.invokeWithTimeout('catalyst:slow-operation', {}, 1000)
