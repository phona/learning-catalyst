...
describe('IPC Communication Integration', () => {
  ...
  describe('Vector Database Operations', () => {
    test('should perform vector operations through knowledge service', async () => {
      ...
      // Initialize knowledge service with proper vector database dependency
      ...
      const knowledgeService = new KnowledgeService(vectorDB);

      // Create concept through knowledge service (uses vector DB internally)
      const conceptData = {
        name: 'React Hooks',
      ...
      expect(concept.id).toMatch(/^concept_/);
  ...
  describe('Knowledge Service Integration', () => {
    test('should integrate knowledge service with database via IPC', async () => {
      ...
      // Initialize vector database module for knowledge service
      const vectorDB = new VectorDatabaseModule();
      await vectorDB.initialize();
      const knowledgeService = new KnowledgeService(vectorDB);
      ...
      // Note: Vector database operations are now handled internally by VectorDatabaseModule
      // through KnowledgeService, not via direct IPC calls
    });
  ...
  describe('Error Handling and Resilience', () => {
    test('should handle IPC timeout errors gracefully', async () => {
      ...
      await expect(database.init()).rejects.toThrow('IPC timeout');
    });
    ...
    test('should handle IPC reconnection scenarios', async () => {
      ...
      // First attempt fails
      await expect(database.init()).rejects.toThrow('Connection lost');
