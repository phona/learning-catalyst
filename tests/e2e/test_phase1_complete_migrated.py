...
class Phase1Tester:
    ...
    def start_application(self):
        ...
        self.process = subprocess.Popen(
            [sys.executable, "-m", "src.cli.main", "start-learning", self.test_dir],
    ...
    def cleanup(self):
        """Clean up the process and test directory"""
        if self.process:
            try:
...
@pytest.fixture
def phase1_tester():
    """Create a Phase1Tester instance for testing"""
    tester = Phase1Tester()
    yield tester
    tester.cleanup()


class TestPhase1Stories:
    """Test class for Phase 1 user stories"""
    ...
    def test_story1_first_time_onboarding(self, phase1_tester):
        ...
        assert help_found, "Help command not working"
    ...
    def test_story5_answer_feedback(self, phase1_tester):
        ...
        ai_feedback = any("AI Tutor:" in line for line in answer_output)
        assert ai_feedback, "AI feedback not found"
    ...
    def test_story6_model_configuration(self, phase1_tester):
        ...
        models_found = any("Available Models" in line or "Configured Models" in line for line in models_output)
        assert models_found, "Models command not working"

        # Test config command
        config_output = phase1_tester.send_command("/config", 3)
        assert config_output, "No config output received"

        config_found = any("AI configuration" in line or "Current configuration" in line for line in config_output)
        assert config_found, "Config command not working"
    ...
    def test_story7_checkpoint_management(self, phase1_tester):
        ...
        assert save_output, "No save output received"

        save_found = any("Saving checkpoint" in line for line in save_output)
        assert save_found, "Checkpoint save not working"

        # Test checkpoint list
        list_output = phase1_tester.send_command("/checkpoint list", 3)
        assert list_output, "No list output received"

        # Test checkpoint load
        load_output = phase1_tester.send_command("/checkpoint load phase1-test", 3)
        assert load_output, "No load output received"

        load_found = any("Loading checkpoint" in line or "Restored" in line for line in load_output)
        assert load_found, "Checkpoint load not working"
    ...
    def test_integration_local_markdown(self, phase1_tester):
        ...
        assert concepts_found, "Concepts command not working"
        ...
        knowledge_map_found = any("Knowledge Map" in line or "Relationships" in line for line in knowledge_map_output)
        assert knowledge_map_found, "Knowledge map command not working"
