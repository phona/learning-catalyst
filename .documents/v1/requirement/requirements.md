...
- **Concept Building System**: (Phase 2 & 3) Extracts concepts from Markdown files, builds knowledge graphs, and tracks proficiency.
...
| `/rebuild`                | (Phase 2) Re-analyzes the user's Markdown files to rebuild the concept knowledge graph with different granularity options. |
| `/status` or `/config`    | (Phase 2) Shows the current configuration and project status. |
...
- **Simplicity**: Configuration is managed through human-readable files.
...
- **Concept Granularity Control**: The system must offer user-selectable modes for concept extraction and knowledge graph construction.
